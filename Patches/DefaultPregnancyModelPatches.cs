...
namespace Dramalord.Patches
{
    ...
    public static class GetDailyChanceOfPregnancyForHeroPatch
    {
        ...
        {
            if(!DramalordMCM.Instance.AllowDefaultPregnancies || DramalordPregnancies.Instance.GetPregnancy(hero) != null)
            {
                __result = 0;
                return false;
            }
            ...
            if(hero.Clan == null)
            {
                ...
                int num3 = hero.Clan?.AliveLords.Count((Hero x) => x.IsAlive) ?? 3;
