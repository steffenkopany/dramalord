...
{
    internal class ConfrontDateIntention : Intention
    {
        [SaveableField(4)]
        ...
        private static ConfrontDateIntention? ConversationInstance() => ConversationTools.ConversationIntention as ConfrontDateIntention;

        public ConfrontDateIntention(Hero target, Hero other, Hero intentionHero, CampaignTime validUntil, bool isWitness) : base(intentionHero, target, validUntil)
        {
            Other = other;
            IsWitness = isWitness;
        }
        ...
        internal static void AddDialogs(CampaignGameStarter starter)
        {
            DialogFlow npcFlow = DialogFlow.CreateDialogFlow("start", 200)
                .BeginNpcOptions()
                    .NpcOption("{npc_starts_confrontation_unknown}[ib:aggressive][if:convo_undecided_open]", () => Hero.OneToOneConversationHero.IsDramalordLegit() && ConversationTools.ConversationIntention as ConfrontDateIntention != null && ConversationTools.ConversationIntention.IntentionHero != Hero.MainHero && !Hero.OneToOneConversationHero.HasMet)
                        .Consequence(() => Hero.OneToOneConversationHero.SetHasMet())
                        .GotoDialogState("confront_date")
                    .NpcOption("{npc_starts_confrontation_known}[ib:aggressive][if:convo_undecided_open]", () => Hero.OneToOneConversationHero.IsDramalordLegit() && ConversationTools.ConversationIntention as ConfrontDateIntention != null && ConversationTools.ConversationIntention.IntentionHero != Hero.MainHero && Hero.OneToOneConversationHero.HasMet)
                        .GotoDialogState("confront_date")
                .EndNpcOptions();

            DialogFlow npcFlow2 = DialogFlow.CreateDialogFlow("confront_date")
                .BeginNpcOptions()
                    .NpcOption("{npc_confrontation_date}[ib:warrior][if:convo_grave]", () => !Hero.OneToOneConversationHero.IsEmotionalWith(Hero.MainHero))
                        .GotoDialogState("confront_date2")
                    .NpcOption("{npc_confrontation_date_player}[ib:warrior][if:convo_grave]", () => Hero.OneToOneConversationHero.IsEmotionalWith(Hero.MainHero))
                        .GotoDialogState("confront_date2")
                .EndNpcOptions();

            DialogFlow npcFlow3 = DialogFlow.CreateDialogFlow("confront_date2")
                .BeginPlayerOptions()
                    .PlayerOption("{npc_reply_confrontation_love}")
                        ...
                        .GotoDialogState("confront_date3")
            ...
            DialogFlow npcFlow4 = DialogFlow.CreateDialogFlow("confront_date3")
                .BeginNpcOptions()
        ...
        private static void ConsequenceConfrontations(Hero hero, Hero target)
        {
            ...
            RelationshipLossAction.Apply(hero, target, out int loveDamage, out int trustDamage, 30, 50);
        ...
        public override void OnConversationStart()
        {
            ...
            ConversationLines.npc_confrontation_date.SetTextVariable("HERO", Other.Name);
            ConversationLines.npc_confrontation_date.SetTextVariable("STATUS", ConversationTools.GetHeroRelation(IntentionHero, Other));
            ConversationLines.npc_confrontation_date_player.SetTextVariable("HERO", Other.Name);
            ConversationLines.npc_confrontation_date_player.SetTextVariable("STATUS", ConversationTools.GetHeroRelation(IntentionHero, Hero.MainHero));
