...
{
    internal class ConfrontIntercourseIntention : Intention
    {
        [SaveableField(4)]
        ...
        private static ConfrontIntercourseIntention? ConversationInstance() => ConversationTools.ConversationIntention as ConfrontIntercourseIntention;

        public ConfrontIntercourseIntention(Hero target, Hero other, Hero intentionHero, CampaignTime validUntil, bool isWitness) : base(intentionHero, target, validUntil)
        {
            Other = other;
            IsWitness = isWitness;
        }
        ...
        internal static void AddDialogs(CampaignGameStarter starter)
        {
            DialogFlow npcFlow = DialogFlow.CreateDialogFlow("start", 200)
                .BeginNpcOptions()
                    .NpcOption("{npc_starts_confrontation_unknown}[ib:aggressive][if:convo_undecided_open]", () => Hero.OneToOneConversationHero.IsDramalordLegit() && ConversationTools.ConversationIntention as ConfrontIntercourseIntention != null && ConversationTools.ConversationIntention.IntentionHero != Hero.MainHero && !Hero.OneToOneConversationHero.HasMet)
                        .Consequence(() => Hero.OneToOneConversationHero.SetHasMet())
                        .GotoDialogState("confront_intercourse")
                    .NpcOption("{npc_starts_confrontation_known}[ib:aggressive][if:convo_undecided_open]", () => Hero.OneToOneConversationHero.IsDramalordLegit() && ConversationTools.ConversationIntention as ConfrontIntercourseIntention != null && ConversationTools.ConversationIntention.IntentionHero != Hero.MainHero && Hero.OneToOneConversationHero.HasMet)
                        .GotoDialogState("confront_intercourse")
                .EndNpcOptions();

            DialogFlow npcFlow2 = DialogFlow.CreateDialogFlow("confront_intercourse")
                .BeginNpcOptions()
                    .NpcOption("{npc_confrontation_intercourse}[ib:warrior][if:convo_grave]", () => !Hero.OneToOneConversationHero.IsEmotionalWith(Hero.MainHero))
                        .GotoDialogState("confront_intercourse2")
                    .NpcOption("{npc_confrontation_intercourse_player}[ib:warrior][if:convo_grave]", () => Hero.OneToOneConversationHero.IsEmotionalWith(Hero.MainHero))
                        .GotoDialogState("confront_intercourse2")
                .EndNpcOptions();

            
            DialogFlow npcFlow3 = DialogFlow.CreateDialogFlow("confront_intercourse2")
                .BeginPlayerOptions()
                    .PlayerOption("{npc_reply_confrontation_love}")
                        ...
                        .GotoDialogState("confront_intercourse3")
            ...
            DialogFlow npcFlow4 = DialogFlow.CreateDialogFlow("confront_intercourse3")
                .BeginNpcOptions()
        ...
        private static void ConsequenceConfrontations(Hero hero, Hero target)
        {
            ...
            RelationshipLossAction.Apply(hero, target, out int loveDamage, out int trustDamage, 50, 50);
        ...
        public override void OnConversationStart()
        {
            ...
            ConversationLines.npc_confrontation_intercourse.SetTextVariable("HERO", Other.Name);
            ConversationLines.npc_confrontation_intercourse.SetTextVariable("STATUS", ConversationTools.GetHeroRelation(IntentionHero, Other));
            ConversationLines.npc_confrontation_intercourse_player.SetTextVariable("HERO", Other.Name);
            ConversationLines.npc_confrontation_intercourse_player.SetTextVariable("STATUS", ConversationTools.GetHeroRelation(IntentionHero, Hero.MainHero));
